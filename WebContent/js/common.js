...
var Container = function(isPagination, targetDivId, tableId, pageSize) {
	...
	object.init = function(headers) {
		...
		object.headerHtml += '<tr><th id="title_head" colspan="'+ object.headArray.length +'"></th></tr>';
				...
				for (var a = 0; a < object.headArray.length; a++) {
					...
					if ((typeof formatter) == 'function') {
						...
						if (code === "defaultRank") {
							...
							var contentStr = eval('result[(i-1)].' + object.headArray[a].code);
							if((!contentStr || contentStr == "undefined") && contentStr != 0) contentStr = "";
							listHtml += '<td align="' + $.trim(align) + '" valign="' + $.trim(valign) + '"  class="' + (isEmptyClassName ? "": className) + (Boolean(noWrap) == true ? " nowrap ": " ") +(isEmptyHalign?"":halign)+ '">' + contentStr + '</td>';
						}
					}
				}
...
///**
// * 局部显示页面,params={'params1':params1,'params2':params2,...}
// * @param div
// * @param url
// */
//function displayPageDetail(div, url, params) {
//	var arh = new ajaxReturnHtml(url);
//	arh.post($(div), params);
//}
...
/**
 * 通过正则表达批量替换字符串
 * @params content：需要替换的字符串
 * @params regex：被替换的内容
 * @params rTxt：替换成的内容
 * */
function replaceByReg(content,regex,rTxt){
	var reg = eval('/\\'+regex+'/g');
	return content.replace(reg,rTxt);
}
